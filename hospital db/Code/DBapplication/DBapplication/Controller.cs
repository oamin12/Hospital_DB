...
namespace DBapplication
{
    public class Controller
    {
        ...
        public DataTable SelectAllDrStats()
        {
            string query = $"SELECT FName,LName,D.ID,DepName,SuperID,Salary FROM Person AS P,Doctors as D,Department as C  where D.PersonID=P.ID AND C.DepID=D.DepID;";
            return dbMan.ExecuteReader(query);
        }
        public DataTable SelectAllNursesStats()
        {
            string query = $"SELECT FName,LName,Salary,N.ID FROM Person AS P,Nurse AS N where PersonID=P.ID;";
            return dbMan.ExecuteReader(query);
        }
        public DataTable SelectAllEmpsStats()
        {
            string query = $"SELECT FName,LName,Salary,Tname FROM Person AS P,Employee,EmployeeType AS ET  where PersonID=P.ID AND EmployeeTypeID=ET.ID ;";
            return dbMan.ExecuteReader(query);
        }
        ...
        public int InsertScanTransaction(string reqID)
        {
            string query = $"INSERT INTO transactions(requestID) values ({reqID});";
            return dbMan.ExecuteNonQuery(query);
        }
        public int UpdateDrSS(string id, string newS)
        {
